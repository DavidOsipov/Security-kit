...
- Adversary goals considered:
  - Prototype pollution (e.g., injecting __proto__ to change behavior on downstream consumers)
...
```ts
createSecureURL("https://example.test", [], params, undefined, { onUnsafeKey: "skip" });
```

- Enforce HTTPS and length:

```ts
createSecureURL("https://example.test", [], { q: "value" }, undefined, { requireHTTPS: true, maxLength: 2048 });
```
