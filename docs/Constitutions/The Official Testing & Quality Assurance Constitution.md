...
> This document is the authoritative, machine-verifiable specification for _how_ quality, security, performance, accessibility, and SEO are proven in this project. Tests are not optional commentary—they are the specification. If a feature or rule cannot be verified by an automated test, it is considered broken.
...
- **Test Implementation (`ServiceCard.astro` example):**
  ...
  test("ServiceCard should not crash with null props", async () => {
    const { container } = await render(ServiceCard, {
      props: { service: null },
    });
...
const objectFromVm = \_\_runInVmJson('return Array.from(new Uint8Array(8));');
...
- **Test Implementation (`StructuredDataEnhancer.astro` example):**
  ...
  test("StructuredDataEnhancer must link Person and WebSite schema correctly", async () => {
    const { container } = await render(StructuredDataEnhancer, {
      props: { contentType: "homepage", data: {} },
    });
...
- **Test Implementation (Conceptual E2E Snapshot):**
  ...
  test("Header component should match the approved snapshot", async ({
    page,
  }) => {
    await page.goto("/");
    const header = await page.locator("header");
    await expect(header).toHaveScreenshot("header-snapshot.png", {
      maxDiffPixels: 100,
    });
  });
  ```
