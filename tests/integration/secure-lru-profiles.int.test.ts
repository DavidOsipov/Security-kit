...
import { getSecureLRUProfiles, resolveSecureLRUOptions, setSecureLRUProfiles } from '../../src/config';
...
describe('SecureLRU profiles exist and resolve', () => {
  ...
  it('should resolve each profile without throwing', () => {
    for (const name of ['balanced','low-latency-lru','throughput-segmented','throughput-segmented-aggressive','read-heavy-lru-coarse']) {
...
describe('Profile sanity — end-to-end basic ops', () => {
  const profiles = ['balanced','low-latency-lru','throughput-segmented','throughput-segmented-aggressive','read-heavy-lru-coarse'] as const;
