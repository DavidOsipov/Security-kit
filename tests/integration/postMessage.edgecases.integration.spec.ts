...
const mockWindow = { postMessage: mockPostMessage, addEventListener: mockAddEventListener, removeEventListener: mockRemoveEventListener } as any;
...
test('oversized JSON payload is rejected when using json wireFormat', () => {
  ...
  expect(() =>
    sendSecurePostMessage({ targetWindow: mockWindow as any, payload, targetOrigin: 'https://example.com', wireFormat: 'json' })
  ).toThrow();
});
...
test('deeply nested payload exceeds depth limit', () => {
  ...
  let dropped = false;
  const listener = createSecurePostMessageListener({ allowedOrigins: ['https://example.com'], onMessage: () => { /* noop */ }, validate: () => true, wireFormat: 'structured' });

  const handler = mockAddEventListener.mock.calls.find((c) => c[0] === 'message')?.[1];
...
test('circular references cause JSON serialization error on json wireFormat', () => {
  const a: any = {};
  a.self = a;
  expect(() => sendSecurePostMessage({ targetWindow: mockWindow as any, payload: a, targetOrigin: 'https://example.com', wireFormat: 'json' })).toThrow();
});

test('unexpected extra property is rejected when allowExtraProps=false', () => {
  let received = null as any;
  const listener = createSecurePostMessageListener({ allowedOrigins: ['https://example.com'], onMessage: (d) => { received = d; }, validate: { a: 'number' } as any, wireFormat: 'structured' });

  const handler = mockAddEventListener.mock.calls.find((c) => c[0] === 'message')?.[1];
  if (!handler) throw new Error('handler not found');

  handler({ origin: 'https://example.com', source: mockWindow, data: { a: 1, extra: 'nope' } });
...
test('opaque origin with reply port accepted only when no locked origin (worker-less case)', () => {
  ...
  const listener = createSecurePostMessageListener({ allowedOrigins: [], onMessage: (d, ctx) => { received = { d, ctx }; }, validate: () => true, wireFormat: 'structured', allowOpaqueOrigin: true });
  const handler = mockAddEventListener.mock.calls.find((c) => c[0] === 'message')?.[1];
  if (!handler) throw new Error('handler not found');

  const fakePort = { postMessage: vi.fn(), constructor: { name: 'MessagePort' } } as any;
  handler({ origin: '', source: mockWindow, data: { foo: 1 }, ports: [fakePort] });
