...
test('worker-like handshake: lock origin at init and reject other origins', () => {
  ...
  const initEvent = { origin: 'https://client.example', data: { type: 'init' }, source: mockWindow, ports: [] } as any;
  ...
  const listener = createSecurePostMessageListener(
    {
      ...
      onMessage: (data, ctx) => {
        ...
        if (ctx && ctx.event && isEventAllowedWithLock(ctx.event as MessageEvent, locked)) {
          acceptedCount++;
        }
      },
      validate: () => true,
  ...
  handler({ origin: 'https://client.example', source: mockWindow, data: { type: 'ping' }, ports: [] });
  // Message from other origin (should be rejected)
  handler({ origin: 'https://attacker.example', source: {}, data: { type: 'ping' }, ports: [] });
  ...
  handler({ origin: '', source: mockWindow, data: { type: 'ping' }, ports: [fakePort] });
...
test('worker handshake round-trip with reply port: reply allowed only when origin matches locked origin', () => {
  ...
  const initEvent = { origin: 'https://app.example', data: { type: 'init' }, source: mockWindow, ports: [] } as any;
  const locked = computeInitialAllowedOrigin(initEvent);
  expect(locked).toBe('https://app.example');

  // Create a fake reply port that will capture replies
  const replies: unknown[] = [];
  const fakePort = { postMessage: (m: unknown) => replies.push(m), constructor: { name: 'MessagePort' } } as any;
      ...
      onMessage: (data, ctx) => {
        try {
          if (ctx && ctx.event && isEventAllowedWithLock(ctx.event as MessageEvent, locked)) {
            ...
            if (ctx.ports && ctx.ports[0]) ctx.ports[0].postMessage({ reply: true, echo: data });
          } else {
  ...
  handler({ origin: 'https://app.example', source: mockWindow, data: { cmd: 'hello' }, ports: [fakePort] });
  ...
  replies.length = 0;
  handler({ origin: 'https://evil.example', source: {}, data: { cmd: 'hello' }, ports: [fakePort] });
...
test('worker handshake rejects malformed origins and treats empty origin with no ports as opaque reject', () => {
  ...
  const badInit = { origin: 'not-a-url', data: {}, source: mockWindow, ports: [] } as any;
  const locked = computeInitialAllowedOrigin(badInit);
  // locked may fall back to location.origin or undefined; ensure no crash
  expect(typeof locked === 'string' || typeof locked === 'undefined').toBe(true);
  ...
  expect(() => handler({ origin: '', source: {}, data: { ok: 1 }, ports: [] })).not.toThrow();
