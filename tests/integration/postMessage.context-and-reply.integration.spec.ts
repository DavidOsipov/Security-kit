  ...
  const listener = createSecurePostMessageListener(
    {
      ...
      onMessage: (data, ctx) => {
        ...
        try {
          if (ctx && ctx.ports && ctx.ports[0]) ctx.ports[0].postMessage({ ok: true, echo: data });
        } catch {
          // ignore
        }
      },
...
test('computeInitialAllowedOrigin and isEventAllowedWithLock emulate worker lock semantics', () => {
  ...
  const evWithOrigin = { origin: 'https://foo.example', ports: [] } as MessageEvent;
  ...
  const savedLocation = typeof location !== 'undefined' ? (location as any) : undefined;
  ...
  expect(typeof initial2 === 'string' || typeof initial2 === 'undefined').toBe(true);
