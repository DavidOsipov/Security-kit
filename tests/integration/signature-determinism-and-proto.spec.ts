...
describe('Signature determinism and proto-pollution handling (integration)', () => {
  ...
  const secretRaw = Buffer.from('deterministic-secret-key-32bytes-minimum-length!!');
  ...
  it('signatures remain identical across object key order permutations', async () => {
    ...
    const ok1 = await verifyApiRequestSignature(input1, nonceStore, { maxSkewMs: 60_000 });
    ...
    const ok2 = await verifyApiRequestSignature(input2, nonceStore, { maxSkewMs: 60_000 });
  ...
  it('proto-pollution keys are ignored and do not affect signatures', async () => {
    ...
    const ok1 = await verifyApiRequestSignature(inputClean, nonceStore, { maxSkewMs: 60_000 });
    expect(ok1).toBe(true);
    ...
    const ok2 = await verifyApiRequestSignature(inputPolluted, nonceStore, { maxSkewMs: 60_000 });
    expect(ok2).toBe(true);
  });
});
