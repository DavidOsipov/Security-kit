...
test('worker validates handshake nonce formats and length', async () => {
  ...
  // Initialize worker
  const initEvent = new MessageEvent('message', { data: { type: 'init', secretBuffer: new ArrayBuffer(32), workerOptions: { allowedNonceFormats: ['base64'], handshakeMaxNonceLength: 16 } }, origin: 'https://example.com' } as any);
  ...
  const good = new MessageEvent('message', { data: { type: 'handshake', nonce: 'YWJj' }, ports: [mockPort] } as any);
  await capturedMessageListener(good);
  expect(mockPort.postMessage).toHaveBeenCalled();

  // Invalid format
  const badFormat = new MessageEvent('message', { data: { type: 'handshake', nonce: 'not-base64!' }, ports: [mockPort] } as any);
  mockPort.postMessage.mockClear();
  await capturedMessageListener(badFormat);
  expect(mockPort.postMessage).toHaveBeenCalledWith({ type: 'error', reason: 'nonce-format-invalid' });

  // Too long
  const tooLong = new MessageEvent('message', { data: { type: 'handshake', nonce: 'Y'.repeat(100) }, ports: [mockPort] } as any);
  mockPort.postMessage.mockClear();
  await capturedMessageListener(tooLong);
  expect(mockPort.postMessage).toHaveBeenCalledWith({ type: 'error', reason: 'nonce-too-large' });
});
