...
import { __test_resetForUnitTests, __test_getSaltFailureTimestamp, __test_setSaltFailureTimestamp, __test_ensureFingerprintSalt } from '../../src/postMessage';
...
describe('ensureFingerprintSalt adversarial cooldown', () => {
  it('development fallback: ensureCrypto rejects -> fallback salt produced and cached', async () => {
    ...
    const ensureCrypto = vi.spyOn(state, 'ensureCrypto').mockImplementation(async () => { throw rejectErr; });
  ...
  it('production cooldown: ensureCrypto rejects -> failure timestamp set and immediate retries fail; after clearing timestamp retry succeeds', async () => {
    ...
    ensureCrypto.mockRestore();
    const resolvedCrypto: Crypto = { getRandomValues: (array: Uint8Array) => array, subtle: undefined } as unknown as Crypto;
    const ensureCryptoSuccess = vi.spyOn(state, 'ensureCrypto').mockResolvedValue(resolvedCrypto as any);
