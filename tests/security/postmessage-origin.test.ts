...
function setupMocks() {
  ...
  vi.mock('../../src/postMessage', () => ({
    ...
    isEventAllowedWithLock: vi.fn((event, lock) => { return event.origin === lock; }),
  }));
...
test('worker rejects messages from wrong origin', async () => {
  ...
  const initEvent = new MessageEvent('message', { data: { type: 'init', secretBuffer: new ArrayBuffer(32) }, origin: 'https://allowed.example' } as any);
  await capturedMessageListener(initEvent);
  mockPostMessage.mockClear();

  // Send sign from disallowed origin
  const badEvent = new MessageEvent('message', { data: { type: 'sign', requestId: 1, canonical: 'x' }, origin: 'https://evil.example' } as any);
