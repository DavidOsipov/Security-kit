...
test('worker enforces token-bucket rate limiting end-to-end', async () => {
  ...
  const initMsg = { type: 'init', secretBuffer: new ArrayBuffer(16), workerOptions: { rateLimitPerMinute: 2, rateLimitBurst: 2 } };

  // Send init (meta shape matches other tests: { origin, ports, event })
  await capturedListener(initMsg, { origin: 'https://example.test', ports: [], event: { origin: 'https://example.test' } });
  ...
  await capturedListener({ type: 'sign', requestId: 1, canonical: 'a' }, { origin: 'https://example.test', ports: [replyPort], event: { ports: [replyPort], origin: 'https://example.test' } });
  if (((replyPort.postMessage as any).mock.calls || []).length === 0 && ((mockPostMessage as any).mock.calls || []).length === 0) {
    console.error('After sign 1 - no posts. reply:', (replyPort.postMessage as any).mock.calls, 'global:', (mockPostMessage as any).mock.calls);
  }
  await capturedListener({ type: 'sign', requestId: 2, canonical: 'b' }, { origin: 'https://example.test', ports: [replyPort], event: { ports: [replyPort], origin: 'https://example.test' } });
  if (((replyPort.postMessage as any).mock.calls || []).length === 0 && ((mockPostMessage as any).mock.calls || []).length === 0) {
    console.error('After sign 2 - no posts. reply:', (replyPort.postMessage as any).mock.calls, 'global:', (mockPostMessage as any).mock.calls);
  }

  // Third request should hit rate-limit immediately (burst exhausted)
  await capturedListener({ type: 'sign', requestId: 3, canonical: 'c' }, { origin: 'https://example.test', ports: [replyPort], event: { ports: [replyPort], origin: 'https://example.test' } });
  if (((replyPort.postMessage as any).mock.calls || []).length === 0 && ((mockPostMessage as any).mock.calls || []).length === 0) {
    console.error('After sign 3 - no posts. reply:', (replyPort.postMessage as any).mock.calls, 'global:', (mockPostMessage as any).mock.calls);
  }
  ...
  const signedCount = results.filter((r: any) => r && r.type === 'signed').length;
  const rateLimitCount = results.filter((r: any) => r && r.reason === 'rate-limit-exceeded').length;
  ...
  await capturedListener({ type: 'sign', requestId: 4, canonical: 'd' }, { origin: 'https://example.test', ports: [replyPort], event: { ports: [replyPort], origin: 'https://example.test' } });
  if (((replyPort.postMessage as any).mock.calls || []).length === 0 && ((mockPostMessage as any).mock.calls || []).length === 0) {
    console.error('After sign 4 - no posts. reply:', (replyPort.postMessage as any).mock.calls, 'global:', (mockPostMessage as any).mock.calls);
  }
  ...
  expect(newest.type === 'signed' || newest.reason === 'rate-limit-exceeded').toBe(true);
});
