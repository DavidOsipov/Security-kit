...
test('worker enforces token-bucket at 60/min (1 token/sec) end-to-end', async () => {
  ...
  const initMsg = { type: 'init', secretBuffer: new ArrayBuffer(16), workerOptions: { rateLimitPerMinute: 60, rateLimitBurst: 3 } };
  ...
  await capturedListener({ type: 'sign', requestId: 1, canonical: 'x' }, { origin: 'https://example.test', ports: [replyPort], event: { ports: [replyPort], origin: 'https://example.test' } });
  await capturedListener({ type: 'sign', requestId: 2, canonical: 'y' }, { origin: 'https://example.test', ports: [replyPort], event: { ports: [replyPort], origin: 'https://example.test' } });
  await capturedListener({ type: 'sign', requestId: 3, canonical: 'z' }, { origin: 'https://example.test', ports: [replyPort], event: { ports: [replyPort], origin: 'https://example.test' } });
  ...
  await capturedListener({ type: 'sign', requestId: 4, canonical: 'a' }, { origin: 'https://example.test', ports: [replyPort], event: { ports: [replyPort], origin: 'https://example.test' } });
  ...
  await capturedListener({ type: 'sign', requestId: 5, canonical: 'b' }, { origin: 'https://example.test', ports: [replyPort], event: { ports: [replyPort], origin: 'https://example.test' } });
  const callsAfter = (replyPort.postMessage as any).mock.calls || [];
  const latest = callsAfter[callsAfter.length - 1];
  expect(latest && (latest[0].type === 'signed' || latest[0].reason === 'rate-limit-exceeded')).toBe(true);
});
