...
describe("adversarial URL tests", () => {
  ...
  describe("resource limiting", () => {
    it("should reject too many path segments", () => {
      const manySegments =
        "https://example.com/" +
        Array.from({ length: 70 }, (_, i) => `segment${i}`).join("/");
      expect(() =>
        createSecureURL(
          "https://example.com",
          Array.from({ length: 70 }, (_, i) => `segment${i}`),
        ),
      ).toThrow();
    });
    ...
    it("should reject too many query parameters", () => {
      ...
      for (let i = 0; i < 300; i++) manyParams[`param${i}`] = `value${i}`;
      expect(() =>
        createSecureURL("https://example.com", [], manyParams),
      ).toThrow();
    });

    it("should reject URL with too many query parameters", () => {
      const urlWithManyParams =
        "https://example.com?" +
        Array.from({ length: 300 }, (_, i) => `param${i}=value${i}`).join("&");
