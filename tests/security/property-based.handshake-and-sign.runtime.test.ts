...
const SEED = process.env.FASTCHECK_SEED ? Number(process.env.FASTCHECK_SEED) : undefined;
...
test('runtime fast-check: structured messages do not crash worker and responses are safe', async () => {
  ...
  const tinyBuffer = fc.uint8Array({ minLength: 0, maxLength: 64 }).map((a) => a.buffer);

  const nestedString = fc.oneof(fc.string(), fc.constantFrom(null, undefined), fc.integer().map(String));
  ...
  const initArb = fc.record({
    ...
    workerOptions: fc.option(fc.record({ rateLimitPerMinute: fc.integer({ min: 0, max: 1000 }) })),
  ...
  const handshakeArb = fc.record({
    ...
    meta: fc.option(fc.dictionary(fc.string(), fc.oneof(fc.string(), fc.integer(), fc.boolean())))
  });
  ...
  const signArb = fc.record({
    ...
    nested: fc.option(fc.record({ a: nestedString, b: fc.array(fc.string(), { maxLength: 5 }) })),
  });

  const destroyArb = fc.record({ type: fc.constant('destroy') });

  const unknownArb = fc.record({ type: fc.oneof(fc.string(), fc.constant(null)), payload: fc.anything() });

  const msgArb = fc.oneof(initArb, handshakeArb, signArb, destroyArb, unknownArb);
  ...
  fc.asyncProperty(msgArb, async (msg) => {
      ...
      if ((msg as any).type === 'handshake' && (msg as any).expectReplyPort) {
        if ((msg as any).useRealPort) {
          try {
            ...
            (realPort as any).postMessage = (realPort as any).postMessage.bind(realPort);
      ...
      try {
        await capturedListener!(event.data, { origin: event.origin, ports: event.ports, event });
      } catch (err) {
        ...
        throw new Error(`Worker threw on input: ${JSON.stringify({ msg, info })}`);
      }
      ...
      if ((msg as any).type === 'handshake' && (msg as any).expectReplyPort) {
        ...
        if (last) {
          try {
            ...
            if (mc) { try { mc.port1.close(); mc.port2.close(); } catch {} }
            return false;
          }
        ...
        if (mc) {
          try {
            ...
            for (const m of drained) {
              try { JSON.stringify(m); } catch { if (mc) { try { mc.port1.close(); mc.port2.close(); } catch {} } return false; }
            }
          } catch (e) {
            if (mc) { try { mc.port1.close(); mc.port2.close(); } catch {} }
            return false;
