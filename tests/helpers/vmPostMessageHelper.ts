...
export function loadPostMessageInternals(opts?: {
  ...
  const scriptInner = new vm.Script(wrapped, { filename: srcPath + '.runner' });
        ...
        for (const key of [
          ...
          if (typeof hostCtx[key] !== 'undefined') {
            ...
            try { sandbox.globalThis[key] = hostCtx[key]; } catch { /* best-effort */ }
          }
        }
        ...
        if (typeof hostCtx.ArrayBuffer !== 'undefined' && typeof hostCtx.ArrayBuffer.isView === 'function') {
          ...
          try { sandbox.ArrayBuffer.isView = hostCtx.ArrayBuffer.isView.bind(hostCtx.ArrayBuffer); } catch {}
          try { sandbox.globalThis.ArrayBuffer = sandbox.ArrayBuffer; } catch {}
        }
        ...
        if (typeof (globalThis as any).setImmediate === 'function') sandbox.setImmediate = (globalThis as any).setImmediate;
        if (typeof (globalThis as any).clearImmediate === 'function') sandbox.clearImmediate = (globalThis as any).clearImmediate;
        try { sandbox.globalThis.setTimeout = sandbox.setTimeout; } catch {}
        try { sandbox.globalThis.clearTimeout = sandbox.clearTimeout; } catch {}
        try { sandbox.globalThis.setInterval = sandbox.setInterval; } catch {}
        try { sandbox.globalThis.clearInterval = sandbox.clearInterval; } catch {}
        ...
        if (typeof (globalThis as any).queueMicrotask === 'function') {
          ...
          try { sandbox.globalThis.queueMicrotask = sandbox.queueMicrotask; } catch {}
        }
        ...
        if (typeof (globalThis as any).performance === 'object') {
          ...
          try { sandbox.globalThis.performance = sandbox.performance; } catch {}
        }
        sandbox.Promise = (globalThis as any).Promise;
        try { sandbox.globalThis.Promise = sandbox.Promise; } catch {}
        ...
        const ret = typeof maybe !== 'undefined'
          ? maybe
        ...
        try {
          ...
          Object.defineProperty(exported, '__vm_last_return', { value: ret, enumerable: false, configurable: true, writable: false });
          const meta = {
            typeof: typeof ret,
            isView: (typeof ArrayBuffer !== 'undefined' && typeof ArrayBuffer.isView === 'function') ? ArrayBuffer.isView(ret as any) : false,
            ctorName: (() => {
              try {
                if (ret && typeof ret === 'object') {
                  const p = Object.getPrototypeOf(ret);
                  if (p && p.constructor && typeof p.constructor.name === 'string') return p.constructor.name;
                }
              } catch {}
              return undefined;
            })(),
          };
          Object.defineProperty(exported, '__vm_last_return_meta', { value: meta, enumerable: false, configurable: true, writable: false });
        } catch {
          /* best-effort */
        }
        return ret;
    ...
    Object.defineProperty(exported, '__execInVm', {
      value: (fnName: string, ...args: any[]) => {
        try {
          ...
          const scriptInner = new vm.Script(code, { filename: srcPath + '.exec' });
          const maybe = scriptInner.runInContext(contextified);
          if (typeof maybe === 'string' && maybe.startsWith('__VM_EXCEPTION__')) throw new Error(maybe.slice('__VM_EXCEPTION__'.length));
    ...
    Object.defineProperty(exported, '__runInVmJson', {
      value: (code: string) => {
        try {
          ...
          const scriptInner = new vm.Script(wrapped, { filename: srcPath + '.runjson' });
          ...
          if (typeof maybe === 'undefined') {
            const setResultScript = new vm.Script(`(function(){ try { globalThis.__vm_runjson_result = (function(){ ${code} })(); globalThis.__vm_runjson_string = JSON.stringify(globalThis.__vm_runjson_result); } catch(e) { globalThis.__vm_runjson_string = '__VM_EXCEPTION__' + String(e); } })()` , { filename: srcPath + '.runjson.set' });
            setResultScript.runInContext(contextified);
            const vmStr = contextified.__vm_runjson_string;
            if (typeof vmStr === 'string' && vmStr.startsWith('__VM_EXCEPTION__')) {
              throw new Error(vmStr.slice('__VM_EXCEPTION__'.length));
            }
            if (typeof vmStr !== 'undefined') {
              maybe = vmStr;
            } else if (typeof contextified.__vm_runjson_result !== 'undefined') {
              try {
          ...
          if (typeof maybe === 'string' && (maybe as string).startsWith('__VM_EXCEPTION__')) {
            throw new Error((maybe as string).slice('__VM_EXCEPTION__'.length));
          }
          try {
            return typeof maybe === 'string' ? JSON.parse(maybe as string) : maybe;
          } catch {
            return maybe;
          }
        } catch (err) {
          ...
          try { return `__RUN_ERROR__${String(err)}`; } catch { return undefined; }
        }
      },
      enumerable: false,
      configurable: true,
  ...
  try {
    if (exported && typeof exported === 'object' && typeof (exported as any).default === 'object') {
      const def = (exported as any).default;
      for (const k of Object.keys(def)) {
        if (typeof (exported as any)[k] === 'undefined') (exported as any)[k] = def[k];
      }
    }
  } catch {
    /* best-effort */
  ...
  try {
    const internals = (exported as any).__test_internals || ((exported as any).default && (exported as any).default.__test_internals);
    if (internals && typeof internals === 'object') {
      if (typeof (exported as any).__test_toNullProto === 'undefined' && typeof internals.toNullProto === 'function') {
        (exported as any).__test_toNullProto = internals.toNullProto.bind(internals);
      }
      if (typeof (exported as any).__test_getPayloadFingerprint === 'undefined' && typeof internals.getPayloadFingerprint === 'function') {
        (exported as any).__test_getPayloadFingerprint = internals.getPayloadFingerprint.bind(internals);
      }
      if (typeof (exported as any).__test_ensureFingerprintSalt === 'undefined' && typeof internals.ensureFingerprintSalt === 'function') {
        (exported as any).__test_ensureFingerprintSalt = internals.ensureFingerprintSalt.bind(internals);
      }
      if (typeof (exported as any).__test_deepFreeze === 'undefined' && typeof internals.deepFreeze === 'function') {
        (exported as any).__test_deepFreeze = internals.deepFreeze.bind(internals);
      }
    }
  } catch {
    /* best-effort */
