...
describe("dom.ts remaining branches", () => {
  ...
  afterEach(async () => {
    ...
    try {
      ...
      try { (m.__test_sha256Hex as any).__test_importOverride = undefined; } catch {}
      try { (m.DOMValidator as any).__test_importOverride = undefined; } catch {}
      try { m.__test_resetDefaultValidatorForUnitTests(); } catch {}
    } catch {
      /* ignore */
    }
    vi.restoreAllMocks();
  ...
  const out = await mod3.__test_sha256Hex("foo");
      ...
      (globalThis as any).crypto = savedCrypto;
      try { delete (mod3.__test_sha256Hex as any).__test_importOverride; } catch {}
    }
  });
  ...
  const v = new mod6.DOMValidator({ auditHook, emitSelectorHash: true } as any);
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  (mod7.__test_sha256Hex as any).__test_importOverride = () => Promise.reject(new Error("no crypto"));
  const v = new mod7.DOMValidator({ auditHook, emitSelectorHash: true } as any);
    const savedCrypto = (globalThis as any).crypto;
    try {
