...
describe('DOMValidator (no-DOM environment)', () => {
  ...
  it('rejects selectors with expensive pseudo-classes', () => {
    expect(() => v.validateSelectorSyntax('div:has(span)')).toThrow(InvalidParameterError);
    expect(() => v.validateSelectorSyntax('input:not([type])')).toThrow(InvalidParameterError);
  });
