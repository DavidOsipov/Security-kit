...
describe("SecureLRUCache regressions and hardening", () => {
  ...
  describe("SharedArrayBuffer detection edge cases", () => {
    it("rejects various SAB-backed typed array types", () => {
      ...
      expect(() => cache.set("u8", new Uint8Array(sab))).toThrow(
        InvalidParameterError,
      );
      expect(() =>
        cache.set("u16", new Uint8Array(new Uint16Array(sab).buffer)),
      ).toThrow(InvalidParameterError);
      expect(() =>
        cache.set("i32", new Uint8Array(new Int32Array(sab).buffer)),
      ).toThrow(InvalidParameterError);
      expect(() =>
        cache.set("f64", new Uint8Array(new Float64Array(sab).buffer)),
      ).toThrow(InvalidParameterError);
    });
    ...
    it("rejects SAB-backed DataView", () => {
      ...
      expect(() => cache.set("dv", new Uint8Array(dataView.buffer))).toThrow(
        InvalidParameterError,
      );
    });
    ...
    it("accepts regular ArrayBuffer-backed views", () => {
      ...
      expect(() =>
        cache.set("u16", new Uint8Array(new Uint16Array(ab).buffer)),
      ).not.toThrow();
      expect(() =>
        cache.set("i32", new Uint8Array(new Int32Array(ab).buffer)),
      ).not.toThrow();
    });
    ...
    it("handles subclassed typed arrays", () => {
      ...
      const subclassedView = Object.setPrototypeOf(
        tempView,
        MyUint8Array.prototype,
      ) as MyUint8Array;

      // Should still detect as SAB-backed
      expect(() => cache.set("subclassed", subclassedView)).toThrow(
        InvalidParameterError,
      );
    });
    ...
    it("respects rejectSharedBuffers=false for all SAB types", () => {
      ...
      expect(() =>
        cache.set("dv", new Uint8Array(new DataView(sab).buffer)),
      ).not.toThrow();
    });
  });
});
