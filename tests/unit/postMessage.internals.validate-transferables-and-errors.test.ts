...
test('validateTransferables rejects MessagePort and typed arrays when disallowed', () => {
  ...
  const toNull = (pm as any).__test_toNullProto ?? ((pm as any).__test_internals?.toNullProto as any);
...
test('fingerprint fallback works when crypto unavailable (dev)', async () => {
  ...
  const pm = loadPostMessageInternals({ stubCrypto: { getRandomValues: (u: Uint8Array) => { for (let i=0;i<u.length;i++) u[i]=i; } }, production: false });
  ...
  const salt = saltRaw instanceof Uint8Array ? saltRaw : new Uint8Array(saltRaw as any);
...
test('sendSecurePostMessage errors on invalid targetOrigin and oversized payload', () => {
  ...
  expect(() => pm.sendSecurePostMessage({ targetWindow: fakeWindow, payload: {}, targetOrigin: '*' })).toThrow();
  // oversized payload
  const big = 'x'.repeat(pm.POSTMESSAGE_MAX_PAYLOAD_BYTES + 10);
  expect(() => pm.sendSecurePostMessage({ targetWindow: fakeWindow, payload: big, targetOrigin: 'https://example.com' })).toThrow();
}, 20000);
