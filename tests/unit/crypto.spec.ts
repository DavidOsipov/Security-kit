...
describe("crypto.ts - core primitives", () => {
  ...
  it("getSecureRandomAsync throws on aborted signal", async () => {
    await expect(
      cryptoModule.getSecureRandomAsync({
  ...
  it("getSecureRandomInt validates params and handles min===max", async () => {
    await expect(cryptoModule.getSecureRandomInt(5, 4)).rejects.toBeInstanceOf(
      InvalidParameterError,
    );
  ...
  it("getSecureRandomInt respects abort signal immediately", async () => {
    await expect(
      cryptoModule.getSecureRandomInt(0, 10, {
        signal: { aborted: true } as AbortSignal,
      }),
    ).rejects.toBeTruthy();
  });
  ...
  it("shouldExecuteThrottled validates probability and returns boolean", async () => {
    expect(() =>
      cryptoModule.shouldExecuteThrottled(-1 as unknown as number),
  ...
  it("generateSecureStringSync handles single-character alphabet and rejects bad alphabets", () => {
    ...
    expect(() => cryptoModule.generateSecureStringSync("", 4)).toThrow(
      InvalidParameterError,
    );
  ...
  it("generateSecureStringAsync produces expected length and respects abort", async () => {
    ...
    await expect(
      cryptoModule.generateSecureStringAsync("abcd", 10, {
        signal: { aborted: true } as AbortSignal,
      }),
    ).rejects.toBeTruthy();
  });
  ...
  it("createOneTimeCryptoKey validates length arguments and usages", async () => {
    ...
    await expect(
      cryptoModule.createOneTimeCryptoKey({ lengthBits: 42 as any }),
    ).rejects.toBeInstanceOf(InvalidParameterError);
    await expect(
      cryptoModule.createOneTimeCryptoKey({ usages: [] as any }),
    ).rejects.toBeInstanceOf(InvalidParameterError);
  });
  ...
  it("createAesGcmNonce validates byteLength and returns correct size", () => {
    ...
    expect(() => cryptoModule.createAesGcmNonce(11)).toThrow(
      InvalidParameterError,
    );
  });
  ...
  it("generateSRI works for string and ArrayBuffer inputs and validates algorithm", async () => {
    ...
    await expect(
      cryptoModule.generateSRI(null as unknown as any),
    ).rejects.toBeInstanceOf(InvalidParameterError);
  });
  ...
  it("secureCompare and secureCompareAsync behave correctly and error on too-long inputs", async () => {
    ...
    expect(() => utils.secureCompare(long, long)).toThrow(
      InvalidParameterError,
    );
    ...
    const ensureCryptoSpy = vi
      ...
      .mockResolvedValue({} as unknown as Crypto);
    await expect(
      utils.secureCompareAsync("a", "a", { requireCrypto: true }),
  ...
  it("getSecureRandomInt throws on range too large for platform", async () => {
    ...
    try {
      await expect(
        cryptoModule.getSecureRandomInt(0, 0x100000000),
      ).rejects.toBeInstanceOf(InvalidParameterError);
    } finally {
      globalThis.BigUint64Array = originalBigUint64Array;
    }
  ...
  it("generateSRI throws when SubtleCrypto.digest is unavailable", async () => {
    ...
    const ensureCryptoSpy = vi
      .spyOn(state, "ensureCrypto")
      .mockResolvedValue({} as Crypto);

    try {
      await expect(cryptoModule.generateSRI("test")).rejects.toBeInstanceOf(
        CryptoUnavailableError,
      );
    } finally {
      ensureCryptoSpy.mockRestore();
