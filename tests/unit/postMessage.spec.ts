...
async function createListener(allowed: any, onMessage?: (d: unknown) => void) {
  ...
  const listener = postMessage.createSecurePostMessageListener(allowed as any, onMessage as any);
...
describe("postMessage utilities", () => {
  ...
  it("validateTransferables allows typed arrays when allowTypedArrays=true", async () => {
    ...
    expect(() => postMessage.validateTransferables(ta, false, true)).not.toThrow();
  });
  ...
  it("toNullProto strips forbidden keys and rejects depth overflow", async () => {
    ...
    const res = postMessage.__test_toNullProto(src, 0, 10) as Record<string, unknown>;
    expect(res.a).toBe(1);
  ...
  it("sendSecurePostMessage JSON path serializes and posts string", async () => {
    ...
    postMessage.sendSecurePostMessage({ targetWindow: win, payload: { x: 1 }, targetOrigin: "https://example.com", wireFormat: "json" });
  ...
  it("sendSecurePostMessage JSON rejects oversized payloads", async () => {
    ...
    try {
      postMessage.sendSecurePostMessage({ targetWindow: win, payload: big, targetOrigin: "https://example.com", wireFormat: "json" });
      expect.fail("Expected InvalidParameterError to be thrown");
    } catch (error) {
      expect(error).toBeInstanceOf(InvalidParameterError);
    }
  ...
  it("sendSecurePostMessage structured path rejects when sanitize + allowTypedArrays incompatible", async () => {
    ...
    const win = { postMessage: (_: unknown, __: string) => {} } as unknown as Window;
    try {
      postMessage.sendSecurePostMessage({ targetWindow: win, payload: new Uint8Array([1]), targetOrigin: "https://example.com", wireFormat: "structured", sanitize: true, allowTypedArrays: true });
      expect.fail("Expected InvalidParameterError to be thrown");
    } catch (error) {
      expect(error).toBeInstanceOf(InvalidParameterError);
    }
  ...
  it("createSecurePostMessageListener JSON path rejects non-string and parses JSON", async () => {
    ...
    const listener = await createListener({ allowedOrigins: ["https://example.com"], onMessage, validate: () => true, wireFormat: "json" });
  ...
  it("createSecurePostMessageListener auto allows same-origin structured data and rejects otherwise", async () => {
    const onMessage = vi.fn();
    const l = await createListener({ allowedOrigins: ["https://example.com"], onMessage, wireFormat: "auto", validate: () => true });
    ...
    const ev = { origin: "https://example.com", data: { x: 1 } } as unknown as MessageEvent;
  ...
  it("createSecurePostMessageListener expectedSource comparator throwing is handled", async () => {
    ...
    const l = await createListener({ allowedOrigins: ["https://example.com"], onMessage, expectedSource: comparator as any });
  ...
  it("structured path validation rejects transferables when not allowed", async () => {
    ...
    const fakePort = Object.create((MessagePort as any).prototype);
    try {
      postMessage.sendSecurePostMessage({ targetWindow: win, payload: fakePort, targetOrigin: "https://example.com", wireFormat: "structured" });
      expect.fail("Expected TransferableNotAllowedError to be thrown");
    } catch (error) {
      expect(error).toBeInstanceOf(TransferableNotAllowedError);
    }
