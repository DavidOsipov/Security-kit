...
describe("url - extra hardening edgecases", () => {
  ...
  it("createSecureURL rejects IPv4 shorthand hostnames", () => {
  ...
  it("createSecureURL rejects authority containing percent-encoding", () => {
    expect(() =>
      createSecureURL("http://exa%25mple.com", [], {}, undefined, {}),
    ).toThrow(InvalidParameterError);
  });

  it("createSecureURL rejects embedded credentials", () => {
    expect(() =>
      createSecureURL("http://user:pass@example.com", [], {}, undefined, {}),
    ).toThrow(InvalidParameterError);
  });

  it("createSecureURL rejects multiple @ characters in authority", () => {
    expect(() =>
      createSecureURL("http://a@b@c.com", [], {}, undefined, {}),
    ).toThrow(InvalidParameterError);
  });
  ...
  it("createSecureURL rejects dangerous query keys when onUnsafeKey=throw", () => {
    ...
    expect(() =>
      createSecureURL("https://example.com", [], m as any, undefined, {
        onUnsafeKey: "throw" as any,
      }),
    ).toThrow(InvalidParameterError);
  });
