...
describe('canonical additional coverage', () => {
  it('filters `constructor` key and other forbidden keys', () => {
    const input = { safe: 1, constructor: { hacked: true }, prototype: { p: 2 } } as any;
    const canon = toCanonicalValue(input) as Record<string, unknown>;
    expect(Object.prototype.hasOwnProperty.call(canon, 'constructor')).toBe(false);
    expect(Object.prototype.hasOwnProperty.call(canon, 'prototype')).toBe(false);
  ...
  it('arrays with functions have functions elided but other elements preserved', () => {
    const input = [1, () => 2, { a: 3, b() { return 4; } }];
    ...
    expect(Object.prototype.hasOwnProperty.call(canon[2] as object, 'b')).toBe(false);
  });
});
