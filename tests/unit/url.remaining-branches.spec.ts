...
describe("url.remaining-branches - IDNA, IPv6, forbidden chars, port/colon, percent-encoding", () => {
  ...
  it("rejects invalid colon usage in authority (multiple colons without brackets)", () => {
    ...
    expect(() => createSecureURL("https://host:80:90")).toThrow(
      InvalidParameterError,
    );
  });
  ...
  it("percent-encoded sequences in path validated when enabled", () => {
    ...
    expect(() => createSecureURL("https://example.com/%GG", ["a"])).toThrow(
      InvalidParameterError,
    );
  });

  it("rejects IPv4-shorthand as ambiguous", () => {
  ...
  it("validateURL and normalizeOrigin reject IPv4 shorthand consistently", () => {
    const res = validateURL("https://192.168.1");
    expect(res.ok).toBe(false);
    expect(() => normalizeOrigin("https://192.168.1")).toThrow(
      InvalidParameterError,
    );
  });
