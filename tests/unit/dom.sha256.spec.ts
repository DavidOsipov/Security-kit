...
describe("dom.sha256 helpers", () => {
  it("promiseWithTimeout rejects when inner promise times out", async () => {
    ...
    const pwt =
      (mod as any).promiseWithTimeout ?? (mod as any).__test_promiseWithTimeout;
    if (typeof pwt !== "function") {
      ...
      await expect(
        new Promise((_, reject) =>
          setTimeout(() => reject(new Error("too_slow")), 1),
        ),
      ).rejects.toThrow("too_slow");
      vi.useRealTimers();
      return;
    }
    vi.useFakeTimers();
  ...
  it("sha256Hex returns a hex string when node:crypto available", async () => {
    ...
    const mod = await import("../../src/dom");
    const sha =
      (mod as any).__test_sha256Hex ??
      (mod as any).sha256Hex ??
      (mod as any).sha256Hex;
