...
describe("SecureLRUCache (standalone class)", () => {
  ...
  describe("construction and validation", () => {
    ...
    it("throws when freezeReturns is true but copyOnGet is false", () => {
      expect(
        () => new SecureLRUCache({ freezeReturns: true, copyOnGet: false }),
      ).toThrow("freezeReturns` requires `copyOnGet` to be true");
    });
  });
  ...
  describe("security features", () => {
    it("rejects SharedArrayBuffer views by default", () => {
      if (typeof SharedArrayBuffer !== "undefined") {
        ...
        expect(() => cache.set("shared", sharedView)).toThrow(
    ...
    it("allows SharedArrayBuffer views when configured", () => {
      if (typeof SharedArrayBuffer !== "undefined") {
        const permissiveCache = new SecureLRUCache({
  ...
  describe("validation and error handling", () => {
    ...
    it("throws on oversized keys", () => {
      ...
      expect(() => cache.set(longKey, new Uint8Array([1]))).toThrow(
        InvalidParameterError,
      );
    });

    it("throws on invalid key type", () => {
      expect(() => cache.set(123 as any, new Uint8Array([1]))).toThrow(
        InvalidParameterError,
      );
    });

    it("throws on invalid value type", () => {
      expect(() => cache.set("key", "not-uint8array" as any)).toThrow(
        InvalidParameterError,
      );
    });
    ...
    it("respects per-operation maxEntryBytes override", () => {
      ...
      expect(() =>
        cache.set("key1", value, { maxEntryBytes: 100 }),
      ).not.toThrow();

      // Should fail with lower limit
      expect(() => cache.set("key2", value, { maxEntryBytes: 25 })).toThrow(
        InvalidParameterError,
      );
    });
  });
  ...
  describe("statistics", () => {
    ...
    it("includes URLs in stats when configured", () => {
      const urlIncludingCache = new SecureLRUCache({
