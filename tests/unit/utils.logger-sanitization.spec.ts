...
describe('utils logger sanitization', () => {
  ...
  describe('sanitizeLogMessage', () => {
    it('redacts JWT-like strings', () => {
      return (async () => {
        ...
        const message = 'User token: eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c';
    ...
    it('truncates very long messages', () => {
      return (async () => {
        const { sanitizeLogMessage, MAX_LOG_STRING } = await import('../../src/utils');
    ...
    it('handles sanitizer errors gracefully', () => {
      return (async () => {
        ...
        const problematic = { toString: () => { throw new Error('test error'); } };
  ...
  describe('sanitizeComponentName', () => {
    ...
    it('rejects invalid component names', () => {
      return (async () => {
        const { sanitizeComponentName } = await import('../../src/utils');
        expect(sanitizeComponentName('component with spaces')).toBe('unsafe-component-name');
        expect(sanitizeComponentName('component@domain')).toBe('unsafe-component-name');
        expect(sanitizeComponentName('component<script>')).toBe('unsafe-component-name');
  ...
  describe('integration with secureDevLog', () => {
    it('sanitizes message and component in secureDevLog', () => {
      return (async () => {
        ...
        secureDevLog('info', 'auth-service', 'Login failed: token=secret123', { user: 'test' });
