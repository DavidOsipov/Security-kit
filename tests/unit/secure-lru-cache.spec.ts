  ...
  class FakeWorker {
    ...
    postMessage(msg: any) {
      ...
      if (msg && msg.type === 'handshake') {
        if (transfer && transfer.length > 0 && typeof (transfer[0] as any)?.postMessage === 'function') {
          setTimeout(() => {
            try {
              (transfer[0] as any).postMessage({ type: 'handshake', signature: 'AAA' });
            } catch {
              // ignore
            }
          }, 1);
        ...
        setTimeout(() => {
          const event = { data: { type: 'handshake', signature: 'AAA' } } as any;
          (this.handlers['message'] || []).forEach((h) => h(event));
        }, 1);
      }
    }
...
describe('VerifiedByteCache eviction and blob URL revocation', () => {
  ...
  it('limits cache size and evicts oldest when full', () => {
    ...
    expect(VerifiedByteCache.get('https://example.com/worker-0.js')).toBeUndefined();
  });
  ...
  it('eviction order determinism: oldest removed first', () => {
    ...
    for (let i = 0; i < 10; i++) {
      VerifiedByteCache.set(`https://example.com/fill-${i}.js`, new Uint8Array([i]));
    }
  ...
  it('concurrent set/get stress test (basic)', async () => {
    VerifiedByteCache.clear();
    const urls = Array.from({ length: 20 }, (_, i) => `https://example.com/c-${i}.js`);

    // Concurrently set entries
    await Promise.all(
      urls.map((u, i) => Promise.resolve().then(() => VerifiedByteCache.set(u, new Uint8Array([i])))),
    );
  ...
  it('LRU recency is respected on get (moves to MRU)', () => {
    VerifiedByteCache.clear();
    for (let i = 0; i < 10; i++) {
      VerifiedByteCache.set(`https://example.com/lru-${i}.js`, new Uint8Array([i]));
    }
    ...
    VerifiedByteCache.set('https://example.com/lru-new.js', new Uint8Array([99]));
    expect(VerifiedByteCache.get('https://example.com/lru-1.js')).toBeUndefined();
  ...
  it('Blob worker uses exact verified bytes (hash equality)', async () => {
    ...
    const usedHash = await (await import('../../src/encoding-utils')).sha256Base64(
      (used as Uint8Array)!.buffer as ArrayBuffer,
    );
