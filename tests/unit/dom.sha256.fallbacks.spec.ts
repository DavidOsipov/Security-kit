...
describe("sha256Hex fallback strategies (deterministic)", () => {
  ...
  afterEach(() => {
    ...
    try {
      if (
        ...
        typeof originalSubtleDigest === "function"
      ) {
  ...
  it("uses node:crypto when available", async () => {
    ...
    vi.mock("node:crypto", () => ({
      createHash: (_alg: string) => ({
        update: (_s: string) => ({
          digest: (_enc?: string) => Buffer.from("aabb", "hex"),
        }),
      }),
    }));

    const mod = await importDomModule();
    const sha = (mod as any).__test_sha256Hex as any as (
      s: string,
    ) => Promise<string>;
  ...
  it("uses hash-wasm when earlier options unavailable", async () => {
    ...
    vi.mock("hash-wasm", () => ({
      sha256: (s: string) => Promise.resolve("hh22"),
    }));
    ...
    (sha as any).__test_importOverride = async (spec: string) => {
      if (spec === "hash-wasm")
        return { sha256: (s: string) => Promise.resolve("hh22") };
      return Promise.reject(new Error("unavailable"));
    };
