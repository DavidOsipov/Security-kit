...
describe("verify-sanitize (unit)", () => {
  ...
  it("findConsoleViolations reports unguarded console", () => {
    ...
    expect(violations.some((s) => s.includes("console.* usage detected"))).toBe(
      true,
    );
  });
  ...
  it("findConsoleViolations allows console inside dev helper", () => {
    const src =
      'function _devConsole() { if (isDevelopment()) { console.log("fine") } }';
  ...
  it("processFallback collects no-console messages", () => {
    const res: any = {
      ...
      messages: [
        { ruleId: "no-console", line: 1, message: "Unexpected console" },
      ],
    };
    const out: string[] = [];
