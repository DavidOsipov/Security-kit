...
test('omitted body and body: undefined produce same stable canonicalization', () => {
  ...
  const objUndefined = { headers: { a: 1 }, method: 'GET', body: undefined } as any;
...
test('canonicalization: omitted body and body: undefined produce same bodyHash', async () => {
  ...
  const payload = { a: 1 };
  const contextWithUndefined = { method: 'GET', path: '/x', body: undefined as unknown };
  const contextOmitted = { method: 'GET', path: '/x' } as any;

  const bodyString1 = safeStableStringify((contextWithUndefined as any).body ?? undefined);
  const bodyString2 = safeStableStringify((contextOmitted as any).body ?? undefined);
