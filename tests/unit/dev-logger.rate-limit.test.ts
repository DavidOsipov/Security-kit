...
test('dev logger rate-limit emits console.warn summary when tokens exhausted', () => {
  ...
  secureDevelopmentLog('info', 'testComponent', 'second dev log (should be dropped)', { b: 2 });
  ...
  const hasRateLimit = calls.some((c) => c.includes('[security-kit] dev log rate-limit: dropping'));
