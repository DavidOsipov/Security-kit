...
import type { InitMessage, SignRequest, SignedResponse, ErrorResponse } from '../../src/protocol';
...
describe('Rate Limiting - Comprehensive Tests', () => {
  ...
  beforeEach(() => {
    ...
    mockWorker.postMessage.mockImplementation((msg: any, transfer?: any[]) => {
      ...
      if (msg.type === 'init') {
      ...
      } else if (msg.type === 'handshake' && transfer && transfer.length === 1) {
        ...
        if (rateLimitPerMinute > 0 && rateLimitTokens <= 0) {
          const err = { type: 'error', requestId, reason: 'rate-limit-exceeded' };
          setImmediate(() => {
            if (port) {
              port.postMessage(err);
              try { port.close(); } catch {}
        ...
        setTimeout(() => {
          ...
          const mockSignature = Buffer.from(new Uint8Array(32).fill(0)).toString('base64');
          const response = { type: 'signed', requestId, signature: mockSignature };
          if (port) {
