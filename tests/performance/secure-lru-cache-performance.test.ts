...
function calculateStats(times: number[]): PerformanceStats {
  ...
  const variance = times.reduce((acc, time) => acc + Math.pow(time - mean, 2), 0) / n;
  ...
  const mom = blockMeans.sort((x, y) => x - y)[Math.floor(blockMeans.length / 2)];
...
function bootstrapCI(samples: number[], statFn: (arr: number[]) => number, iters = 500, alpha = 0.05) {
...
describe('SecureLRUCache Performance Tests', () => {
  ...
  describe('Statistical Performance Analysis', () => {
    it('should perform SET operations with consistent performance', { timeout: 60000 }, () => {
      const times: number[] = [];
  ...
  const bootstrapIters = Number(process.env.PERF_BOOTSTRAP_ITERS || '500');
  const momCI = bootstrapCI(times, momEstimator, bootstrapIters);
  const p95CI = bootstrapCI(times, (arr) => calculateStats(arr).p95, bootstrapIters);
  ...
  expect(p95CI.hi).toBeLessThan(PERFORMANCE_BUDGETS.SET_TIME_P95_MS);
    ...
    it('should perform GET operations with consistent performance (high hit-rate)', { timeout: 60000 }, () => {
      const times: number[] = [];
      const data = new Uint8Array(1024);
      const iterations = PERFORMANCE_BUDGETS.MIN_SAMPLES;
  ...
  const bootstrapItersG = Number(process.env.PERF_BOOTSTRAP_ITERS || '500');
  const momCIG = bootstrapCI(times, momEstimator, bootstrapItersG);
  const p95CIG = bootstrapCI(times, (arr) => calculateStats(arr).p95, bootstrapItersG);
  ...
  expect(p95CIG.hi).toBeLessThan(PERFORMANCE_BUDGETS.GET_TIME_P95_MS);
    ...
    it('should perform DELETE operations with consistent performance and actually remove entries', { timeout: 60000 }, () => {
      const times: number[] = [];
      const data = new Uint8Array(1024);
      const iterations = PERFORMANCE_BUDGETS.MIN_SAMPLES;
  ...
  const bootstrapItersD = Number(process.env.PERF_BOOTSTRAP_ITERS || '500');
  const momCID = bootstrapCI(times, momEstimator, bootstrapItersD);
  const p95CID = bootstrapCI(times, (arr) => calculateStats(arr).p95, bootstrapItersD);
  ...
  describe('Mixed Workload Performance', () => {
    it('should handle mixed read/write workload efficiently', { timeout: 30000 }, () => {
  ...
  expect(setStats.mom).toBeLessThan(PERFORMANCE_BUDGETS.BULK_OPERATIONS_MEAN_MS);
  expect(getStats.mom).toBeLessThan(PERFORMANCE_BUDGETS.BULK_OPERATIONS_MEAN_MS);
      expect(setStats.p95).toBeLessThan(PERFORMANCE_BUDGETS.BULK_OPERATIONS_P95_MS);
      expect(getStats.p95).toBeLessThan(PERFORMANCE_BUDGETS.BULK_OPERATIONS_P95_MS);
  ...
  describe('Eviction Performance Analysis', () => {
    it('should perform LRU evictions with consistent timing', { timeout: 30000 }, () => {
      ...
      expect(stats.p95).toBeLessThan(PERFORMANCE_BUDGETS.EVICTION_TIME_P95_MS);
  ...
  describe('Memory and Throughput Analysis', () => {
    it('should demonstrate memory efficiency metrics', { timeout: 15000 }, () => {
      ...
      const memoryEfficiency = (entries * data.length) / finalStats.totalBytes;
      const overheadPerEntry = (finalStats.totalBytes - (entries * data.length)) / entries;
      
      // Memory efficiency should be reasonable (>80% of actual data)
      expect(memoryEfficiency).toBeGreaterThan(0.8);
      expect(finalStats.totalBytes).toBeLessThanOrEqual(PERFORMANCE_BUDGETS.MAX_MEMORY_BYTES);
  ...
  describe('bench-lru derived tests', () => {
    it('key-shape sensitivity: numeric-like vs short vs long string keys', { timeout: 20000 }, () => {
      const data = new Uint8Array(256);
      const iterations = 20000;

      function runWithKeyBuilder(keyBuilder: (i: number) => string) {
        const testCache = new SecureLRUCache({ maxEntries: iterations * 2, maxBytes: iterations * 2 * data.length });
        // Warmup
      ...
      const longStrings = runWithKeyBuilder(i => `key-${i}-${'x'.repeat(512)}`);

      console.log('Key-shape metrics:', {
        numericLike: { mean: numericLike.mean.toFixed(4), p95: numericLike.p95.toFixed(4), cv: numericLike.cv.toFixed(2) + '%' },
        shortStrings: { mean: shortStrings.mean.toFixed(4), p95: shortStrings.p95.toFixed(4), cv: shortStrings.cv.toFixed(2) + '%' },
        longStrings: { mean: longStrings.mean.toFixed(4), p95: longStrings.p95.toFixed(4), cv: longStrings.cv.toFixed(2) + '%' }
      });

      // Assert that non-pathological short string keys are similar or better than long strings
      expect(shortStrings.p95).toBeLessThan(longStrings.p95 * 1.5);
    });
    ...
    it('large-value GC and eviction impact', { timeout: 30000 }, () => {
      ...
      const testCache = new SecureLRUCache({ maxEntries: 100, maxBytes: 5 * largeSize });
  ...
  console.log('Large-value eviction stats (MoM):', { mom: stats.mom.toFixed(4), p95: stats.p95.toFixed(4), p99: stats.p99.toFixed(4) });
      // p95 should be bounded reasonably (avoid extreme GC spikes masking eviction cost)
      expect(stats.p95).toBeLessThan(200);
    });

    it('feature overhead: copyOnSet / copyOnGet impact', { timeout: 20000 }, () => {
      const data = new Uint8Array(1024);
      const runs = 2000;

      function bench(opts: any) {
        const c = new SecureLRUCache({ maxEntries: runs * 2, maxBytes: runs * 2 * data.length, copyOnSet: !!opts.copyOnSet, copyOnGet: !!opts.copyOnGet });
        // warmup
  ...
  console.log('Feature overhead (MoM ms):', { both: both.mom.toFixed(4), none: none.mom.toFixed(4) });
