...
function run(fn:()=>void, samples=200){
  ...
  for(let i=0;i<samples;i++){
    const t0 = now(); fn(); const t1 = now(); out.push(t1-t0);
  }
  return out;
}

function median(a:number[]){ const s=[...a].sort((x,y)=>x-y); const m=Math.floor(s.length/2); return s.length%2===0?(s[m-1]+s[m])/2:s[m]; }

describe('crypto security perf', ()=>{
  test('getSecureRandomBytesSync throughput', ()=>{
    const samples = run(()=>{ getSecureRandomBytesSync(256); }, 300);
  ...
  test('getSecureRandomInt stress', async ()=>{
    const out:number[]=[];
    for(let i=0;i<200;i++){
      const t0=now(); await getSecureRandomInt(0, 1000000); const t1=now(); out.push(t1-t0);
    }
  ...
  test('secureWipe cost', ()=>{
    ...
    const samples = run(()=>{ secureWipe(buf); }, 200);
    const med = median(samples);
    expect(med).toBeLessThan(2);
  });
});
