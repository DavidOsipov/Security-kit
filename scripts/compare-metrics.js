...
if (argv.length < 2) {
  console.error(
    "Usage: compare-metrics.js <testMetrics.json> <benchResult.json>",
  );
  process.exit(2);
}
...
const benchKeyCandidates = [
  "SecureLRU (sieve tuned)",
  "SecureLRU",
  "SecureLRU (tuned)",
];
let benchEntry = null;
...
for (const op of ops) {
  ...
  if (diff == null) console.log("  delta: -");
    ...
    console.log(
      `  delta: ${diff.toFixed(1)}% ${Math.abs(diff) > 25 ? "<< SIGNIFICANT" : ""}`,
    );
  console.log("");
}
...
if (
  ...
  benchEntry.SET &&
  benchEntry.SET.opsPerSec
) {
  const ttp = test.throughput.opsPerSec;
  const btp = benchEntry.SET.opsPerSec;
  const pd = percentDiff(ttp, btp);
  console.log(
    `Throughput: test ${Math.round(ttp).toLocaleString()} ops/sec vs bench ${Math.round(btp).toLocaleString()} ops/sec`,
  );
  console.log(
    ` delta: ${pd.toFixed(1)}% ${Math.abs(pd) > 25 ? "<< SIGNIFICANT" : ""}`,
  );
}
...
if (benchEntry._debug || test._debug) {
  ...
  console.log(" test :", td);
  if (
    (bd.sieveScans || 0) + (bd.sieveRotations || 0) === 0 &&
    (td.sieveScans || 0) + (td.sieveRotations || 0) > 0
  ) {
    console.log(
