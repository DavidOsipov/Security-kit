...
async function main() {
  ...
  const cache = new SecureLRUCache({ maxEntries: 1000, maxBytes: 1024 * 1024, maxEntryBytes: 32 * 1024 });
  ...
  bench.addEventListener('cycle', (evt: any) => {
    ...
    const r = task?.result;
    if (r && r.latency) {
      console.log(`${name} — mean: ${r.mean.toFixed(6)} ms  p95: ${r.p95?.toFixed(6)} ms  p99: ${r.p99?.toFixed(6)} ms`);
    }
  });
  ...
  const mean = r.mean ?? (r.latency && r.latency.mean) ?? null;
    ...
    const samples: number[] = Array.isArray(r.samples) ? r.samples : (Array.isArray(r.latency?.samples) ? r.latency.samples : []);
  ...
  results[name] = { mean, p50, p95, p99, samples: samples.length, outliers, opsPerSec };

    console.log(`\n=== ${name} ===`);
    if (typeof mean === 'number') console.log(`mean: ${mean.toFixed(6)} ms`);
    if (typeof mean === 'number') console.log(`ops/sec: ${Math.round(1000 / mean).toLocaleString()}`);
    if (typeof p50 === 'number') console.log(`p50: ${p50.toFixed(6)} ms  p95: ${p95?.toFixed(6)} ms  p99: ${p99?.toFixed(6)} ms`);
    console.log(`samples: ${samples.length}  top-outliers(ms): ${outliers.map(x=>x.toFixed(6)).join(', ')}`);
  }

  // write results JSON
  try {
    const fn = `benchmarks/results-secure-lru-cache-${Date.now()}.json`;
    await import('fs').then(m=>m.promises.writeFile(fn, JSON.stringify(results, null, 2), 'utf8'));
