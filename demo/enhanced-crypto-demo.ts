...
import { ensureCrypto, secureRandomBytes, isCryptoAvailable } from '../src/state';
...
async function demoEnhancedCrypto() {
  ...
  if (available) {
    ...
    console.log(`✅ Crypto provider detected: ${crypto.constructor.name || 'Crypto'}`);
    ...
    console.log(`   Sample: ${Array.from(testArray.slice(0, 8)).map(b => b.toString(16).padStart(2, '0')).join(' ')}`);
  }
