...
const tokenCache = new SecureLRUCache({
  ...
  onEvict: (entry) => {
    console.log(
      `🔒 Token evicted: ${entry.url.replace("token:", "")} (${entry.reason})`,
    );
  },
});
...
const smallCache = new SecureLRUCache({
  ...
  onEvict: (entry) => {
    console.log(
      `🗑️  Evicted due to ${entry.reason}: ${entry.url} (${entry.bytesLength} bytes)`,
    );
  },
});
...
setTimeout(() => {
  ...
  console.log(
    `📊 TTL Cache stats: ${stats.expired} expired, ${stats.size} remaining`,
  );
}, 1500);
